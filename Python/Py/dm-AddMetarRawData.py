    ...
    if not os.path.exists(dir):                                                                 # <Raw Data> folder not found
        while True:
            ...
            if dir_input.lower() == 'y':                                                        # continue
                ...
                print("No folder created. Exit run...")                                         # terminal notification
                time.sleep(1.5)
                exit()                                                                          # exit program
            else:
...
def countLocalFiles():
    count_local = len(metar_alist) - len(local_alist)                                           # (metar - local) deviation
    if len(local_alist) < len(metar_alist):                                                     # default: metar > local
        print (f"{count_local} files will be added. Start scraping...")
        time.sleep(1)
        return count_local
...
def parseMetarSpeci(soup: BeautifulSoup):
    if soup.find("span", {"class":"data_text"}):                                                # content returns data
        ...
        if soup.find("select", {"name":"page_number_1"}):                                       # multiple page contents
            ...
            fetchData()                                                                         # single page contents
...
for items in metar_alist:                                                                       # loop list of metar_speci.php airport list
    if items not in local_alist:                                                                # files not existing in local repo
        ...
        with open(ms.params["icao"]+".txt", "a+", encoding="utf-8") as file:                    # open file <append mode>, utf-8 encode (WARE: 01/10/2022 data)
            count += 1
            template = f"({count:0>3}/{count_local:0>3})"                                       # terminal display for counting list
